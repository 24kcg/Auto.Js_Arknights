//如果屏幕没有点亮，则唤醒设备。
device.wakeUpIfNeeded();sleep(3000);
swipe(300,1160,600,300,2000);sleep(3000);

var volume=device.getMusicVolume()//返回当前媒体音量
toastLog("当前媒体音量："+volume);sleep(2000);
//设置当前媒体音量
//此函数需要"修改系统设置"的权限。如果没有该权限，会抛出SecurityException并跳转到权限设置界面。
device.setMusicVolume(0); 
toastLog("当前媒体音量设为0");sleep(2000);
...
toastLog("35*1000毫秒后瞎点几下（1秒=1000毫秒）");
for (var j = 35; j > 0; j--) {
...
toastLog("等待60*1000毫秒（1秒=1000毫秒）");
for (var j = 60; j > 0; j--) {
    toastLog(j); sleep(1000);
}
...
if (TorF) {
    ...
    if (TorF) {
        ...
        if (TorF) {
            找图_通用("无人机加速_最多", "右上"); sleep(5000);
            找图_通用("无人机加速_确定", "右下"); sleep(5000);
    ...
    if (TorF) {
        ...
        if (d.getHours() <= 12) {//晚上要录视频，所以改了 原：>12
            ...
            TorF = 找图_通用("基建换班_制造站", "全屏", "x+200_y+50");
            if (TorF) {
                TorF = 找图_通用("蛇屠箱", "全屏", "true");
                找图_通用("基建换班_清空选择", "左下");
                if (TorF) {
                    //从右往左滑一点
                    swipe(height / 2, width / 2, height / 2.5, width / 2, 2000); sleep(2000);
                    找图_通用("刻俄柏");
                    找图_通用("红云");
                    找图_通用("蛇屠箱");
                    //滑回去
                    swipe(height / 2, width / 2, height, width / 2, 2000); sleep(2000);
                } else {
                    找图_通用("史都华德");
                    找图_通用("芬");
                    找图_通用("稀音");
...
TorF = 找图_通用("导航_作战", "上半屏");
if (TorF) {
    for (var l = 15; l > 0; l--) {
        toastLog(l); sleep(1000);
    }

    找图_通用("物资筹备", "下半屏"); sleep(3000);
    if (d.getDay() == 1 || d.getDay() == 4 || d.getDay() == 6 || d.getDay() == 0) {
        toastLog("周 " + d.getDay() + "，刷红票"); sleep(2000);
        找图_通用("粉碎防御", "下半屏"); sleep(3000);
        找图_通用("AP-5", "右上"); sleep(3000);
        刷图();
    } else {
        toastLog("周 " + d.getDay() + "，经验（LS-5)");sleep(2000);
        click(280, 350);sleep(3000);
        click(1056, 180);sleep(3000);
        刷图();
    }
...
toast("还原媒体音量："+volume);sleep(2000);

toastLog("结束喽~");
// 回收图片（image对象创建后尽量在不使用时进行回收）例外的是，caputerScreen()返回的图片不需要回收。
img_small.recycle();sleep(3000);
